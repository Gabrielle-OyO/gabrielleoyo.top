...
<h1 class="theme-plane">vCards后台设置面板</h1>
    ...
    <p>开源地址：<a href="https://github.com/Gabrielle-OyO">Github</a></p>
    <p>欢迎大家对本模板进行star~</p>
    <p>作者博客：<a href="https://github.com/Gabrielle-OyO">Gabrielle</a></p>
    <p>欢迎加微信：<a href="https://github.com/Gabrielle-OyO">GABRIELLE_OYO</a></p>

</div>
EOF;
echo '<script src="' . Helper::options()->themeUrl . '/assets/styles/options.js"></script>';
    $headTitle = new Typecho_Widget_Helper_Form_Element_Text('headTitle', NULL, "Gabrielle_OyO", _t('首页头部昵称'), _t('请输入你的昵称'));
    $headTitle->setAttribute('class', 'options-content options-home');
    $form->addInput($headTitle);
    $headStatus = new Typecho_Widget_Helper_Form_Element_Text('headStatus', NULL, "Gabrielle_OyO！", _t('首页头部一句话介绍'), _t('请输入你的介绍，用于展示在首页昵称下面'));
    ...
    $logoUrl = new Typecho_Widget_Helper_Form_Element_Text('logoUrl', NULL, "http://116.62.211.242/usr/themes/vCards/logo.png", _t('首页头部头像地址'), _t('在这里填入一个图片 URL 地址, 以在头部设置你的头像'));
    $logoUrl->setAttribute('class', 'options-content options-home');
    $form->addInput($logoUrl);
    $headbgImgUrl = new Typecho_Widget_Helper_Form_Element_Text('headbgImgUrl', NULL, "http://116.62.211.242/usr/themes/vCards/screenshot.png", _t('首页头部背景图片地址'), _t('在这里填入一个图片 URL 地址, 在首页显示头部背景'));
    ...
    $hd_qq = new Typecho_Widget_Helper_Form_Element_Text('hd_qq', NULL, "https://github.com/Gabrielle-OyO", _t('qq'), _t('请输入联系qq'));
    $hd_qq->setAttribute('class', 'options-content options-contact');
    $form->addInput($hd_qq); 
    $hd_email = new Typecho_Widget_Helper_Form_Element_Text('hd_email', NULL, "https://github.com/Gabrielle-OyO", _t('邮箱'), _t('请输入邮箱地址'));
    $hd_email->setAttribute('class', 'options-content options-contact');
    $form->addInput($hd_email); 
    $hd_github = new Typecho_Widget_Helper_Form_Element_Text('hd_github', NULL, "https://github.com/Gabrielle-OyO", _t('github地址'), _t('请输入github地址'));
    $hd_github->setAttribute('class', 'options-content options-contact');
    $form->addInput($hd_github); 
    $hd_weibo = new Typecho_Widget_Helper_Form_Element_Text('hd_weibo', NULL, "https://github.com/Gabrielle-OyO", _t('微博'), _t('请输入微博地址'));
    $hd_weibo->setAttribute('class', 'options-content options-contact');
    $form->addInput($hd_weibo); 
    $hd_coolapk = new Typecho_Widget_Helper_Form_Element_Text('hd_coolapk', NULL, "https://github.com/Gabrielle-OyO", _t('酷安'), _t('请输入酷安地址'));
    $hd_coolapk->setAttribute('class', 'options-content options-contact');
    $form->addInput($hd_coolapk); 
    $hd_wx = new Typecho_Widget_Helper_Form_Element_Text('hd_wx', NULL, "http://116.62.211.242/gabrielleoyo.top/theam/vCards/assets/vxGABRIELLE_OYO.jpg", _t('微信'), _t('请输入微信联系截图'));
